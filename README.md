...
Comments from AJ:
I have written my code on mac using python. I have only run my code on mac 10.9.5
You should be able to run this python file like: python ./truss_sample.py sample_broken_utf_8.csv 
The sample file is the file provided by Truss. I have committed the sample files also.
I am very new to Python. I have been trying to solve problems in python for a few weeks now.
I would have preferred to not have decode on every column of the csv file but rather have it decoded when I am opening up the file but it was giving me errors. I wasted some time looking into it.
I also did not run decode on FooDuration, BarDuration, TotalDuration
I also spent some time (wasted time) on ISO-8601 conversion to EST time

Assumptions:
The order of the csv is the same
Preferred to have done:
Clean up by running decode to replace or ignore invalid utf-8 characters on file open. Right now I am running it on each column on csv
Added more comments
Would have run it on mac and linux versions mentioned above
